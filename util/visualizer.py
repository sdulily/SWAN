    ...
    def __init__(self, opt):
        ...
        if self.use_html:
            if opt.stage == 'first':
                ...
                self.web_dir = os.path.join(opt.checkpoints_dir, opt.name, 'web_few_shot_1')
                self.img_dir = os.path.join(self.web_dir, 'images_few_shot_1')
                print('create web directory %s...' % self.web_dir)
                util.mkdirs([self.web_dir, self.img_dir])
        if opt.stage == 'first':
            self.log_name = os.path.join(opt.checkpoints_dir, opt.name, 'loss_log.txt')
        elif opt.stage == 'second':
            self.log_name = os.path.join(opt.checkpoints_dir, opt.name, 'loss_log_few_shot_1.txt')
